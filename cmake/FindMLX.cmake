...
set(_MLX_METAL_CPP_HINTS
    "${MLX_INCLUDE_DIR}/metal_cpp"
    "${MLX_ROOT}/include/metal_cpp"
)
foreach(_MLX_BUILD_METADATA_DIR IN LISTS _MLX_BUILD_METADATA_DIRS)
    list(APPEND _MLX_METAL_CPP_HINTS
        "${_MLX_BUILD_METADATA_DIR}/_deps/metal_cpp-src")
    if(EXISTS "${_MLX_BUILD_METADATA_DIR}/CMakeCache.txt")
        file(STRINGS "${_MLX_BUILD_METADATA_DIR}/CMakeCache.txt"
            _MLX_METAL_CPP_SOURCE REGEX "^FETCHCONTENT_SOURCE_DIR_METAL_CPP:PATH=.+")
        if(_MLX_METAL_CPP_SOURCE)
            string(REGEX REPLACE "^[^=]+=" "" _MLX_METAL_CPP_SOURCE
                "${_MLX_METAL_CPP_SOURCE}")
            list(APPEND _MLX_METAL_CPP_HINTS "${_MLX_METAL_CPP_SOURCE}")
        endif()
    endif()
endforeach()
find_path(MLX_METAL_CPP_INCLUDE_DIR NAMES Metal/Metal.hpp
    HINTS ${_MLX_METAL_CPP_HINTS})
